public class FiniteAutomatonMatcher {
  ...
  public int FASearch(String text,String pattern)
  {
   ...
   int[][] TFT = new int [m+1][256];//a transition function table that uses given pattern's length plus one as number of rows
                                    //and  number of all valid unicode characters as number of columns
   int occurrence = 0;
   char T[] = new char [n];
   ...
   int i; 
   int j=0;
   for (i = 0; i < n; i++)//transfer the text to transition table
   {
        j = TFT[j][T[i]];
        ...
        occurrence++;//checks if pattern is found inside text
  ...
  
  {
   ...
   TFT[0][P[0]] = 1;//make first variable in first row equal to 1
     ...
     for (i = 0; i<m; i++)//write in the rows of the transition table 
